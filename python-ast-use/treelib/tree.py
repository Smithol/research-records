...
class Tree(object):
    ...
    def __print_backend(self, nid=None, level=ROOT, idhidden=True, filter=None,
        ...
        # legacy ordering
        #2021-10-14 16:45:55: 其实注释了也没事,key=None没问题的.
        if key is None:
            def key(node):
                return node
    ...
    def __get_iter(self, nid, level, filter_, key, reverse, dt, is_last):
        ...
        if filter_(node) and node.expanded:
            ...
            if key:#2021-10-14 16:19:31: 原来你在这里,注意list.sort(key=None,reverse=False)的效果.
    ...
    def to_graphviz(self, filename=None, shape='circle', graph='digraph'):
        ...
        nodes, connections = [], []
        if self.nodes:
            for n in self.expand_tree(mode=self.WIDTH,sorting=False):
            #for n in self.expand_tree(mode=self.WIDTH):#2021-10-14 19:12:02: 修改expand_tree.
                nid = self[n].identifier
                state = '"{0}" [label="{1}", shape={2}]'.format(
                    nid, trans(self[n].tag), shape)
                nodes.append(state)
        ...
        if not is_plain_file:
            print(f.getvalue())
            #graph_data = f.getvalue()#liji:添加返回值
        f.close()
        #return [graph_data,None][is_plain_file]#liji:添加return